<template>
  <div class="order">
    ...
    <div class="status">
      ...
      <p v-if="info.status==9999">订单状态：客服评审未通过！</p>
      <p v-if="info.status==10">订单状态：评审通过下料中！</p>
      <p v-if="info.status==20">订单状态：待发货！</p>
      <p v-if="info.status==50">订单状态：待收货！</p>
      <p v-if="info.status==90">订单状态：待同步！</p>
      <p v-if="info.status==99">订单状态：已完成！</p>
      <p v-if="info.status==-1">订单状态：取消</p>
    ...
    <div class="main">
      ...
      <div class="divCommon">
        ...
        <div class="proList" v-for="(item,index) in info.orderItemList" :key="item.id">
          ...
          <div class="content">
            <p>系列：{{item.productCatName}}</p>
            <p>形状：{{item.productShapeName}}</p>
            <p v-if="item.productShapeCode==99">形状描述：{{item.shapeRemark}}</p>
            <p v-if="item.productShapeCode==99">形状图片：
              <span>
            ...
            <p>功能：{{item.productFunName}}</p>
            <p v-if="item.status===-1">生产状态：取消</p>
            <p v-if="item.status===0 && item.status===null">生产状态：审核中</p>
            <p v-if="item.status===5">生产状态：已同步待生产</p>
            <p v-if="item.status===10">生产状态：订单下料中</p>
            <p v-if="item.status===20">生产状态：订单组装中</p>
            <p v-if="item.status===30">生产状态：订单喷漆中</p>
            <p v-if="item.status===40">生产状态：订单包装中</p>
            <p v-if="item.status===50">生产状态：订货已发货</p>
          </div>
          <p class="title">{{index+1}}、产品参数</p>
          <div class="content">
            <p v-for="(vals,i) in item.productAttributeList" :key="vals.attributeKeyCode">{{vals.attributeKeyName}}：{{vals.attributeValueName}}</p>
          </div>
          ...
          <div class="content">
            <p>单价(元) ：{{item.productPrice}}</p>
            <p>数量：{{item.orderNum}}</p>
            <p>金额(元)：{{item.discountPrice}}</p>
          </div>
...
<script>
  export default {
    ...
    methods:{
      ...
      toLogistics(){
        let url = '/pages/order/logistics/main?id='+this.info.id
